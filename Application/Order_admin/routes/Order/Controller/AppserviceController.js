...
var AppServiceController = {
  ...
  doChangeSpaceInfo: function (req, res) {
    ...
    cancelSpaceStock: function (req, res) {
        request(Base.mergeRequestOptions({
            method: 'post',
  ...
  cargoutPackageDiId: function (req, res) {
    var id = req.params.id;
    request(Base.mergeRequestOptions({
      method: 'get',
      url: '/api/whse/app/cargoout/package/list/diId/'+id+'?'+ queryString.stringify(req.query),
      headers: req.headers,
    }, req, res), function (error, response, body) {
      if (!error) {
        res.send(JSON.parse(body));
